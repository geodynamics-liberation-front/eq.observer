try:
    ...
    print("smbus2 not available, some functions may not work.")
import struct
import sys
import time
...
def readUShort(register_high,register_low):
    return bus.read_byte_data(ADDR,register_high)<<8 | bus.read_byte_data(ADDR,register_low)

def readShort(register_high,register_low):
    x = bus.read_byte_data(ADDR,register_high)<<8 | bus.read_byte_data(ADDR,register_low)
...
def calibrateAccelX():
    writeAccelOffsetX(calculateOffset(register.XA_OFFSET_H,register.XA_OFFSET_L,readAccelX))

def calibrateAccelY():
    writeAccelOffsetY(calculateOffset(register.YA_OFFSET_H,register.YA_OFFSET_L,readAccelY))

def calibrateAccelZ():
    writeAccelOffsetZ(calculateOffset(register.ZA_OFFSET_H,register.ZA_OFFSET_L,readAccelZ,0x7ff))
...
def readAccelData():
    ...
    while record:
        ...
        print("%d: %f,%f,%f"%(t,x*SCALE_2G,y*SCALE_2G,z*SCALE_2G))
#        print("%d: %d,%d,%d"%(t,x,y,z))
        record=f.read(STRUCT_IN.size)
    end = time.time()
    f.close()
...
def test():
    ...
    print("Accel Data:")
    configAccel(ACCEL_FS_SEL_2G)
    xyz=(readAverage(readAccelX,100), readAverage(readAccelY,100), readAverage(readAccelZ,100))
    print("2g  : %d,%d,%d"%xyz)
